...
void RendererDX9::HandleInputDX9(float dt) {
    ...
    // apply raw mouse movement
    float xoff = g_rawMouseX * cam->mouseSensitivity;
    float yoff = g_rawMouseY * cam->mouseSensitivity;
    g_rawMouseX = g_rawMouseY = 0.0f;

    cam->rotation.y += glm::radians(xoff);
    cam->rotation.x += glm::clamp(glm::radians(yoff), -glm::radians(89.0f), glm::radians(89.0f));

    // derive movement vectors
    glm::mat4 rotM = glm::yawPitchRoll(cam->rotation.y, cam->rotation.x, cam->rotation.z);
    glm::vec3 forward = glm::normalize(glm::vec3(rotM * glm::vec4(0,0,-1,0)));
    glm::vec3 right   = glm::normalize(glm::cross(forward, glm::vec3(0,1,0)));
    glm::vec3 up      = glm::vec3(0,1,0);

    // WASDQE
    if (GetAsyncKeyState('W') & 0x8000) cam->position += forward * cam->movementSpeed * dt;
    if (GetAsyncKeyState('S') & 0x8000) cam->position -= forward * cam->movementSpeed * dt;
    if (GetAsyncKeyState('A') & 0x8000) cam->position -= right   * cam->movementSpeed * dt;
    if (GetAsyncKeyState('D') & 0x8000) cam->position += right   * cam->movementSpeed * dt;
    if (GetAsyncKeyState('Q') & 0x8000) cam->position -= up      * cam->movementSpeed * dt;
    if (GetAsyncKeyState('E') & 0x8000) cam->position += up      * cam->movementSpeed * dt;
}
...
void RendererDX9::RenderFrame() {
    ...
    d3dDevice->SetTransform(D3DTS_PROJECTION, &ToD3D(projGL));

    // view
    glm::mat4 viewGL = glm::lookAt(cam->position, cam->lookAtPosition, glm::vec3(0,1,0));
